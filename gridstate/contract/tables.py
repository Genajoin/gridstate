"""Схема контракта данных SE: ``SEInput`` / ``SEOutput`` (Фаза 0).
...
реально читает и пишет. Поведение пайплайна она пока не меняет (см.
``docs/se_target_architecture.md`` — Фаза 0); схема фиксирует границу, на которую
последующие фазы переведут внутренности.

**Минимальность.** В контракт входят ТОЛЬКО колонки, которые gridstate реально
трогает (карта потребления — аудит рефактора Input/Output). Огромный «хвост»
``PowerSystemModel`` (динамика генераторов, ПА/ЛАПНУ, сечения, виртуальные
группы, таблицы тестовой/целевой системы, ``kct``/``ves``/``type_ekv`` и т.п.) намеренно НЕ
часть контракта.
...
* :attr:`Role.WORKING` — вход, который препроцессинг **мутирует** по ходу
  прогона (статусы, режим, шунты, tap, начальные V/δ). Сегодня живёт во входных
  таблицах как черновик; **Фаза 4** копирует именно эти колонки в явный рабочий
  слой (поэтому роль выделена отдельно).
...
**Типы — у нас.** dtype-строки колонок зафиксированы здесь (контракт самодостаточен,
см. план §5), а не импортируются из ``power-system-core``. Пока PSC ещё в рантайме,
тест-страж сверяет их с PSC-DTYPE (``tests/test_contract.py``), чтобы контракт
оставался верен тому, что код фактически читает/пишет.
"""
...
# ===========================================================================
#
# ВАЖНО (Блокатор 4): FORMULE/ON_LINE/ARG (топология, телеметрия, РПН-спеки,
# материализация) сегодня парсятся НАПРЯМУЮ из XML-файла (``xml_path``), а не из
# raw_tables — типизированные ``nested_formulas``/``formula_args`` это lossy-
# проекция (теряется GENERATOR.gen_num, схлопываются NP/PARALLEL у LINE). Поэтому
# полноценно завести FORMULE-вход в контракт — задача поздней фазы (обогащение
# адаптера загрузки). См. ``docs/input_output_separation.md`` раздел «Блокатор 4».
