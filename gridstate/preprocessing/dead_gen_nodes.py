...
def disable_dead_generator_nodes(
    ...
    *,
    ...
    ⚠️ ПАРКОВАНО (default OFF). Правило net-neutral: гасит терминальные ген-узлы, но
    без статуса повышающей ветви во входных данных оно одинаково часто чинит и
    ломает расхождения, поэтому отключено по умолчанию. Физика свёртки
    переиспользуема, когда во входе появится per-branch-статус ветвей.
