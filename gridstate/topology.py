...
**Фаза 3 плана** ``docs/se_target_architecture.md`` — снять алгоритмическую
зависимость от ``power_system.topology``. Функции перенесены сюда **дословно**
(та же логика, те же литералы типов узлов) → поведение бит-в-бит совпадает с
оригиналом PSC (страж эквивалентности — ``tests/test_topology_port.py``).

**Фаза 4 (слайс 4.1) — решающее ядро над контрактными массивами.** Каждая
функция расщеплена на:
...
* ``_*`` **чистое ядро** — принимает контрактные numpy-массивы
  ...
  ``PowerSystemModel`` и от способа записи;
...
Когда working-слой Ф4 перестанет быть моделью (clone уберётся), адаптер сменит
лишь способ применения плана (``model.update`` → присваивание в рабочий массив);
само ядро останется неизменным.
...
_SLACK_NODE_TYPE = 2  # NodeType.SLACK; literal оставлен чтобы не тянуть utils.constants.
_PQ_NODE_TYPE = 0  # NodeType.PQ
_PV_NODE_TYPE = 1  # NodeType.PV


# ---------------------------------------------------------------------------
# Чистые ядра над контрактными массивами (PSC-free, возвращают план id)
# ---------------------------------------------------------------------------
